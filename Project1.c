...
int dyn1=0,dyn2=0,dyn3=0;	//dyn1--> 1 to disable and 0 to enable autosave, dyn2--> key switch for menu/list linking and dyn3--> key switch to decide print message while program exit
...
void creddedassignself()  	//manual	 //credit/deduction assigning/updating function---> A.1.3
{
	...
	switch(j)
	{
		case 1:printf("\nEnter amount of money to credit: ");scanf("%lf",&mon);o[i].inc+=mon;o[i].cred+=mon;fileautosave();break;
		case 2:printf("\nEnter amount of money to deduct: ");scanf("%lf",&mon);o[i].inc-=mon;o[i].ded+=mon;fileautosave();break;
		default :visor=1;goto LB1;
	}
...
void empadd()	//manual  	//adding an employee detail in runtime function-->	A.3
{
	...
	fflush(stdin);
	printf("Enter employee's name, age, post, mailid, phone number:\n",pos);
	scanf(" %[^\n] %d %s %[^\n] %s", &o[pos-1].name, &o[pos-1].age, &o[pos-1].post,o[pos-1].mailid,o[pos-1].phno);
	o[pos-1].id=uid;
	uid++;
	postissuehandler();
	o[pos-1].ppwd[0]='\0';
...
void mainmenu()		//auto save toggle, read previously saved data or create new data and initialise toggle, linking with sub-menus
{
	...
	if(s<1||s>100)
	{
	 printf("\nInvalid size of array provided(input size exceeds limits of predefined size=[1,100])\n");
	  ...
	  switch(i)
	  {
		case 1:printf("\nEnter a new size(specify new no.of elements to store): ");goto LB1;
		case 2:return;
	...
	{
	  fflush(stdin);
	  printf("enter emp%d name, age, post, mailid and phone no.:\n",i+1);
	  scanf(" %[^\n] %d %s %[^\n] %s",&o[i].name,&o[i].age,&o[i].post,&o[i].mailid,&o[i].phno);
...
void empredirect()	//Employee page		//empaccess linking
{
	...
	if(i==s)
	{
		printf("\n\nThe details provided by you does not match with any employee's detail in the database\n\n\n");
		return;
	}
	fflush(stdin);
...
void main()		//details FYI	//login page
{
	int i=1;
	printf("\n\t\t\t\t\t\t\t\t\t\t\t\t\t\tWELCOME!\n\n");
		...
		case 1:
			{
				...
				printf("It is strongly advised to take extreme care while you input Employee data when the program runs as in some cases program will shut down abnormally.\n");
				printf("Invalid data inputs can not be handled when a wrong type of data is assigned.\n\n");
			}break;
		case 2:break;
		default :visor=1;goto LB2;	
	}
